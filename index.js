...
function download(url, redirectCount) {
	return new Promise((c, e) => {
		var content = "";
		https
			.get(getOptions(url), function (response) {
				...
				response
					...
					.on("end", function () {
						if (
							...
							e(
								"GitHub API rate exceeded. Set GITHUB_TOKEN environment variable to increase rate limit.",
							);
							return;
						}
...
function getCommitSha(repoId, repoPath) {
	var commitInfo =
		"https://api.github.com/repos/" + repoId + "/commits?path=" + repoPath;
	return download(commitInfo).then(function (content) {
		try {
			...
			return Promise.reject(
				new Error("Failed extracting the SHA: " + content),
			);
		}
	});
}
	...
	return download(contentPath)
		.then(function (content) {
			...
			if (ext === ".tmLanguage" || ext === ".plist") {
				...
				return Promise.reject(
					new Error("Unknown file extension: " + ext),
				);
			}
			if (modifyGrammar) {
			...
			return getCommitSha(repoId, repoPath).then(function (info) {
				let result = {
					information_for_contributors: [
						"This file has been converted from https://github.com/" +
							repoId +
							"/blob/master/" +
							repoPath,
				...
				try {
					...
					let cgmanifestContent = fs
						.readFileSync("./cgmanifest.json")
						.toString();
					...
					if (currentCommitDate > commitDate) {
						...
						for (
							let i = 0;
							i < cgmanifestRead.registrations.length;
							i++
						) {
							if (
								cgmanifestRead.registrations[
									i
								].component.git.repositoryUrl.substr(
									cgmanifestRead.registrations[i].component
										.git.repositoryUrl.length -
										repoId.length,
									repoId.length,
								) === repoId
							) {
								cgmanifestRead.registrations[
									i
								].component.git.commitHash = info.commitSha;
								commitDate = currentCommitDate;
								promises.push(
									download(packageJsonPath).then(
					...
					Promise.all(promises).then(function (allResult) {
						...
						fs.writeFileSync(
							"./cgmanifest.json",
							JSON.stringify(cgmanifestRead, null, "\t").replace(
								/\n/g,
								eol,
							),
						);
...
function getExistingEOL(content) {
	for (let i = 0; i < content.length; i++) {
		...
		if (ch === CharCode_CR) {
			if (
				i + 1 < content.length &&
				content.charCodeAt(i + 1) == CharCode_LF
			) {
				return "\r\n";
			}
			return "\r";
